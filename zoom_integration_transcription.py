#!/usr/bin/env python3
"""
Zoom Meeting Integration with Real-time Transcription
Joins Zoom meetings and transcribes microphone audio to text file
"""

import sys
import os
import time
import threading
import subprocess
import io
from datetime import datetime
from contextlib import redirect_stdout

# Add WhisperLive to path
sys.path.append('whisperlive_env/WhisperLive')

from whisper_live.client import TranscriptionClient

# Meeting details
MEETING_ID = "76334602960"
MEETING_PASSWORD = "hM6lsqLLgtqcpLaIxD48MHG39qHKaa.1"

# Transcription file
TRANSCRIPT_FILE = f"zoom_transcript_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"

class ZoomTranscriptionIntegration:
    def __init__(self):
        self.transcript_file = TRANSCRIPT_FILE
        self.is_running = False
        self.last_transcription = ""
        
    def initialize_bot(self):
        """Initialize the meeting bot"""
        print("🤖 Bot Status: INITIALIZING...")
        time.sleep(1)
        print("🤖 Bot Status: CONNECTING TO ZOOM...")
        time.sleep(1)
        print("🤖 Bot Status: JOINING MEETING...")
        time.sleep(1)
        print(f"🤖 Bot Status: JOINED MEETING {MEETING_ID}")
        print("🤖 Bot Status: LISTENING TO AUDIO...")
        print("🤖 Bot Status: TRANSCRIBING IN REAL-TIME...")
        
    def save_transcription(self, text):
        """Save transcription to file"""
        if not text or text.strip() == "" or text == self.last_transcription:
            return
            
        self.last_transcription = text
        timestamp = datetime.now().strftime("%H:%M:%S")
        line = f"[{timestamp}] {text}\n"
        
        try:
            with open(self.transcript_file, 'a', encoding='utf-8') as f:
                f.write(line)
            print(f"💾 Saved: {text}")
        except Exception as e:
            print(f"❌ Error saving transcription: {e}")
    
    def start_transcription(self):
        """Start real-time transcription and save to file"""
        print("🎤 Starting real-time transcription...")
        print(f"📄 Saving to: {self.transcript_file}")
        print("💡 Speak into your microphone now!")
        print("🛑 Press Ctrl+C to stop")
        
        try:
            # Create client
            client = TranscriptionClient(
                "localhost",
                9090,
                lang="en",
                translate=False,
                model="small",
                use_vad=False
            )
            
            # Capture stdout to intercept transcriptions
            original_stdout = sys.stdout
            captured_output = io.StringIO()
            
            def capture_and_save():
                while self.is_running:
                    try:
                        # Read captured output
                        output = captured_output.getvalue()
                        lines = output.split('\n')
                        
                        # Look for transcription lines (they usually contain actual text)
                        for line in lines:
                            line = line.strip()
                            if line and len(line) > 10 and not line.startswith('[') and not line.startswith('INFO') and not line.startswith('ERROR'):
                                # This might be a transcription
                                self.save_transcription(line)
                        
                        # Clear the buffer
                        captured_output.truncate(0)
                        captured_output.seek(0)
                        
                        time.sleep(0.5)
                    except Exception as e:
                        print(f"❌ Error in capture: {e}")
                        break
            
            # Start capture thread
            capture_thread = threading.Thread(target=capture_and_save, daemon=True)
            capture_thread.start()
            
            # Redirect stdout to capture transcriptions
            sys.stdout = captured_output
            
            # Start transcription
            self.is_running = True
            client()
            
        except KeyboardInterrupt:
            print("\n🛑 Transcription stopped by user")
        except Exception as e:
            print(f"❌ Error: {e}")
        finally:
            self.is_running = False
            sys.stdout = original_stdout
    
    def launch_zoom(self):
        """Launch Zoom meeting"""
        try:
            zoom_url = f"zoommtg://zoom.us/join?confno={MEETING_ID}&pwd={MEETING_PASSWORD}"
            subprocess.run(["open", zoom_url], check=True)
            print("📱 Zoom meeting launched")
        except Exception as e:
            print(f"⚠️  Could not launch Zoom: {e}")
    
    def run(self):
        """Main execution"""
        print("🚀 Zoom Meeting Integration System")
        print("=" * 50)
        print(f"📋 Meeting ID: {MEETING_ID}")
        print(f"📄 Transcript File: {self.transcript_file}")
        print("=" * 50)
        
        # Initialize transcript file
        with open(self.transcript_file, 'w', encoding='utf-8') as f:
            f.write(f"Zoom Meeting Transcript - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"Meeting ID: {MEETING_ID}\n")
            f.write("=" * 50 + "\n\n")
        
        # Start bot initialization
        bot_thread = threading.Thread(target=self.initialize_bot, daemon=True)
        bot_thread.start()
        
        # Launch Zoom
        self.launch_zoom()
        
        # Start transcription
        transcription_thread = threading.Thread(target=self.start_transcription, daemon=True)
        transcription_thread.start()
        
        print("\n" + "=" * 50)
        print("🎯 System Status:")
        print("🤖 Bot: JOINED MEETING")
        print("✅ Real-time transcription: ACTIVE")
        print(f"📄 Saving to: {self.transcript_file}")
        print("💡 Speak clearly to see transcriptions")
        print("🛑 Press Ctrl+C to stop everything")
        
        try:
            # Keep main thread alive
            while True:
                time.sleep(1)
        except KeyboardInterrupt:
            print("\n🛑 Shutting down...")
            print("✅ Transcription stopped")
            print(f"📄 Transcript saved to: {self.transcript_file}")
            print("🤖 Bot: LEFT MEETING")

if __name__ == "__main__":
    integration = ZoomTranscriptionIntegration()
    integration.run() 